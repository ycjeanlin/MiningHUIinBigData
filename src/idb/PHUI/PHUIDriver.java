...
public class PHUIDriver {
	...
	public static void main(String[] args) throws Exception {
		...
		while(minUtility >= minMinUtility){
			...
			MRTime = pmHUI.runPHUI(InFile);
			endTime = System.currentTimeMillis();
			totalTime = (endTime - startTime);
			
			numCandidate = pmHUI.exportResult("PHUI_exp/HUI_list_t_"+df.format(minUtility)+"_"+OutFile, false);
	...
	private static void resultExport(double minUtility, long time,
			List<Long> MRTime, long numCandidate) {
		BufferedWriter bw = null;
		
		try {
			bw = new BufferedWriter(new FileWriter("PHUI_exp/time_"+OutFile,true));
		...
		try {
			bw = new BufferedWriter(new FileWriter("PHUI_exp/MRTime_"+OutFile,true));
			for(long t:MRTime){
		...
		try {
			bw = new BufferedWriter(new FileWriter("PHUI_exp/candidate_"+OutFile,true));
